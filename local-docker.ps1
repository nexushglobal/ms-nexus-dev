# local-docker.ps1 - Script con soporte para hot reload
param([string]$Command, [string]$Service)

if (-not $Command) {
    Write-Host "Uso: .\local-docker.ps1 comando [servicio]" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "Ejemplos:"
    Write-Host "  .\local-docker.ps1 up"
    Write-Host "  .\local-docker.ps1 logs gateway"
    Write-Host "  .\local-docker.ps1 restart user-service"
    Write-Host "  .\local-docker.ps1 shell gateway"
    exit
}
...
switch ($Command.ToLower()) {
    "build" {
        ...
        Write-Host "Imagenes construidas! Ahora ejecuta: .\local-docker.ps1 up" -ForegroundColor Green
    }
    ...
    "up" {
        ...
        Write-Host ""
        Write-Host "Para ver logs en tiempo real: .\local-docker.ps1 logs" -ForegroundColor Cyan
    ...
    "restart" {
        if (-not $Service) {
            ...
            Write-Host "Ejemplo: .\local-docker.ps1 restart gateway" -ForegroundColor Yellow
            exit 1
        }
    ...
    "install" {
        if (-not $Service) {
            ...
            Write-Host "Ejemplo: .\local-docker.ps1 install gateway" -ForegroundColor Yellow
            exit 1
        }
        Write-Host "Instalando dependencias en $Service..." -ForegroundColor Blue
        docker-compose -f docker-compose.local.yml exec $Service pnpm install
